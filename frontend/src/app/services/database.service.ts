...
export class DatabaseService {
  ...
  public addUserFormToDatabase() {
    ...
    this.httpClient.post<{status: string}>(environment.custom.ADD_USER_DATA, tempFormData, {observe: 'response'})
       .subscribe(response => {
  ...
  public updateUserFormToDatabase() {
    ...
    this.httpClient.post<{status: string}>(environment.custom.UPDATE_USER_DATA, tempFormData, {observe: 'response'})
       .subscribe(response => {
        if (response.body.status === 'ok') {
          // tost
          // console.log(response.body.status);
