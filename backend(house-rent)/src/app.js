    ...
    form.on('field', (fieldName, fieldValue, file) => {
        ...
        if (Object.keys(userInfo).length === 4 && count === 0) {
            ...
            let data = {
                ...
                password: JSON.stringify(encryption_decryption.encrytion(userInfo.password)),
    ...
    mongodbClient.connect(process.env.DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
        ...
        collection.find({email: {$eq: data.email}}).toArray((err, result) => {
            if(err){
                ...
                if(data.password === encryption_decryption.decryption(Buffer.from(JSON.parse(result[0].password).data))) {
                    res.status(201).json({
                        email: req.body.email,
                        status: 'ok'
                    });
    ...
    form.on('field', (fieldName, fieldValue, file) => {
        ...
        if (Object.keys(ownerInfo).length >=11 && count === 0) {
            ...
            mongodbClient.connect(process.env.DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
                ...
                collection.find({email: {$eq: ownerInfo.email}}).toArray((err, result) => {
                    if(err){
                        console.log(`"${ownerInfo.email}" doesn't match with the API key!`);
                    }else{
                        if(result.length === 0) {
                        ...
                        collection.updateOne({email: {$eq: ownerInfo.email}}, {
                            $push: {
                                ownerInfo: {
                                    $each: [
                                        {
...
app.post('/updateUserData', (req, res, next) => {
    ...
    form.on('field', (fieldName, fieldValue, file) => {
        ...
        // console.log(Object.keys(ownerInfo).length);
            // console.log(tempOldData);
        if (Object.keys(ownerInfo).length >=14 && count === 0 && Object.keys(tempOldData).length > 0) {
            console.log(ownerInfo);
            // console.log(tempOldData);
            mongodbClient.connect(process.env.DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
                if(err){
                    ...
                    if(err){
                        ...
                        result.forEach(res => {
                            res.ownerInfo.forEach(res2 => {
                                // console.log(tempOldData);
                                if (res2.personal.ownerName === tempOldData.ownerInfo.ownerName &&
...
app.post('/allData', (req, res, next) => {
    ...
    mongodbClient.connect(process.env.DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
        ...
        collection.find({email: {$ne: req.body.email}}).toArray((err, result) => {
            if(err){
                console.log(err);
...
app.post('/userData', (req, res, next) => {
    ...
    mongodbClient.connect(process.env.DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
        ...
        collection.find({email: {$eq: req.body.email}}).toArray((err, result) => {
            if(err){
                console.log(err);
                console.log(`"${req.body.email}" doesn't match with the API key!`);
                // return false;
